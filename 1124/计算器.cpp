...
int main()
{
	char s1[100]; //最大长度是 固定的 你也可  自己看着办
	char s2[100];
	while(1)
	{
		gets(s1); //字符串形式 输入
		...
		{ //当年用了 比较蠢的方式 其实可以直接抛字符串 不用 在这里写了
			if(key==1)
			{
			    system("cls");
				cout<<"输入的计算式括号有误，请重新输入";
			}
			else if(key==2)
			{system("cls");
				cout<<"输入的计算式是空的，请重新输入";
			}
			else if(key==3)
			{
			    system("cls");
				cout<<"输入的式子含有非法字符，请重新输入";
				
			}
			else if(key==4)
			{system("cls");
				cout<<"输入的式子格式不正确，请重新输入";
			...
			{system("cls");
				cout<<"阶乘计算的内容只能为正整数，请检查式子";
				
		    }
			else if(key==8)
			{system("cls");
				cout<<"负数的小数次方无意义，请检查式子";
